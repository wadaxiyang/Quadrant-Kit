...
def reusable(record, scene, path):
    if not record or record.get('scene') != scene or not path.is_file():
        return False
    try:
...
def main():
    ...
    common = {**before, 'os': platform.platform(), 'architecture': platform.machine(),
              ...
              'page': args.page, 'preview': args.preview}
    ...
    for width,height,theme,scale,case,compact in cells:
        ...
        info = png_info(fresh)
        fresh.replace(png)
        record = {'schema_version':1,'scene':scene,'image':info,'generated_at_utc':datetime.now(timezone.utc).isoformat()}
