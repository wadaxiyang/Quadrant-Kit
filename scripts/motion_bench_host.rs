...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let count = std::env::args()
        ...
        .parse::<i32>()?;
    let ui = MotionBench::new()?;
    ui.set_count(count);
    let renders = Rc::new(Cell::new(0u64));
    let copy = renders.clone();
    let hook = ui
        .window()
        ...
        .is_ok();
    println!(
        "ENV count={count} hook_supported={hook} logical_cpus={}",
        ...
        started.start(slint::TimerMode::Repeated,Duration::from_millis(16),move||{
            ...
            if index<200 {
                ...
                if index % 20 == 19 {
                    ...
                    println!("MEMORY sample={index} private_bytes={private} working_set_bytes={working}");
                }
                tick.set(index+1);
            } else {
                ...
                slint::Timer::single_shot(Duration::from_secs(1),move||{
                    ...
                    slint::Timer::single_shot(Duration::from_secs(60),move||{
                        ...
                        let (private, working) = memory_bytes();
                        println!("MEMORY_FINAL private_bytes={private} working_set_bytes={working}");
